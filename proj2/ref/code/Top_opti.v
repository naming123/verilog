...
//============================================================================
//  16-point 1D-DCT, stage 1  (row DCT)  -- AREA-OPTIMIZED, bit-identical
//  Even part recursively decomposed (Chen-style butterflies):
//    k0,k8   : pure shift (x256)            -> 0 multipliers
//    k4,k12  : 2 shared terms x {334,138}   -> 4 multipliers
//    k2..k14 : 4x4 on 2nd-stage diffs       -> 16 multipliers
//    odd 8x8 : 64 multipliers
//  total 84 constant mults (was 128), all constants CSD-friendly
//============================================================================
module dct16_stage1 (
    input  [16*8-1:0]  i_x,
    ...
    // ---- 2nd butterfly (even part only) ----
    wire signed [11:0] ss [0:3];
    wire signed [11:0] sd [0:3];
    generate
        for (i = 0; i < 4; i = i + 1) begin : BFLY2
            assign ss[i] = s[i] + s[7-i];
            assign sd[i] = s[i] - s[7-i];
        end
    endgenerate

    // ---- 3rd butterfly (even-even) ----
    wire signed [12:0] u0 = ss[0] + ss[3];
    wire signed [12:0] u1 = ss[1] + ss[2];
    wire signed [12:0] v0 = ss[0] - ss[3];
    wire signed [12:0] v1 = ss[1] - ss[2];

    wire signed [22:0] ze [0:7];
    wire signed [22:0] zo [0:7];

    // k=0,8 : x256 = <<8  (no multiplier)
    assign ze[0] = (u0 + u1) <<< 8;                       // k=0
    assign ze[4] = (u0 - u1) <<< 8;                       // k=8
    // k=4,12 : 4 mults
    assign ze[2] = 334*v0 + 138*v1;                       // k=4
    assign ze[6] = 138*v0 - 334*v1;                       // k=12
    // k=2,6,10,14 : 16 mults
    assign ze[1] = 356*sd[0]+302*sd[1]+202*sd[2]+ 70*sd[3]; // k=2
    assign ze[3] = 302*sd[0]- 70*sd[1]-356*sd[2]-202*sd[3]; // k=6
    assign ze[5] = 202*sd[0]-356*sd[1]+ 70*sd[2]+302*sd[3]; // k=10
    assign ze[7] =  70*sd[0]-202*sd[1]+302*sd[2]-356*sd[3]; // k=14

    // odd rows : unchanged 8x8
    assign zo[0] = 360*d[0]+346*d[1]+320*d[2]+280*d[3]+230*d[4]+170*d[5]+106*d[6]+ 36*d[7];
    assign zo[1] = 346*d[0]+230*d[1]+ 36*d[2]-170*d[3]-320*d[4]-360*d[5]-280*d[6]-106*d[7];
    assign zo[2] = 320*d[0]+ 36*d[1]-280*d[2]-346*d[3]-106*d[4]+230*d[5]+360*d[6]+170*d[7];
    assign zo[3] = 280*d[0]-170*d[1]-346*d[2]+ 36*d[3]+360*d[4]+106*d[5]-320*d[6]-230*d[7];
    assign zo[4] = 230*d[0]-320*d[1]-106*d[2]+360*d[3]- 36*d[4]-346*d[5]+170*d[6]+280*d[7];
    assign zo[5] = 170*d[0]-360*d[1]+230*d[2]+106*d[3]-346*d[4]+280*d[5]+ 36*d[6]-320*d[7];
    assign zo[6] = 106*d[0]-280*d[1]+360*d[2]-320*d[3]+170*d[4]+ 36*d[5]-230*d[6]+346*d[7];
    assign zo[7] =  36*d[0]-106*d[1]+170*d[2]-230*d[3]+280*d[4]-320*d[5]+346*d[6]-360*d[7];

    generate
        for (i = 0; i < 8; i = i + 1) begin : PACK
            assign o_z[16*14-1-14*(2*i)   -: 14] = ze[i] >>> 8;
            assign o_z[16*14-1-14*(2*i+1) -: 14] = zo[i] >>> 8;
        end
    endgenerate
endmodule
...
//============================================================================
//  16-point 1D-DCT, stage 2  (column DCT)  -- AREA-OPTIMIZED, bit-identical
//============================================================================
module dct16_stage2 (
    input  [16*14-1:0] i_x,
    ...
    wire signed [15:0] ss [0:3];
    wire signed [15:0] sd [0:3];
    generate
        for (i = 0; i < 4; i = i + 1) begin : BFLY2
            assign ss[i] = s[i] + s[7-i];
            assign sd[i] = s[i] - s[7-i];
    ...
    wire signed [16:0] u0 = ss[0] + ss[3];
    wire signed [16:0] u1 = ss[1] + ss[2];
    wire signed [16:0] v0 = ss[0] - ss[3];
    wire signed [16:0] v1 = ss[1] - ss[2];
    ...
    assign ze[0] = (u0 + u1) <<< 8;
    assign ze[4] = (u0 - u1) <<< 8;
    assign ze[2] = 334*v0 + 138*v1;
    assign ze[6] = 138*v0 - 334*v1;
    assign ze[1] = 356*sd[0]+302*sd[1]+202*sd[2]+ 70*sd[3];
    assign ze[3] = 302*sd[0]- 70*sd[1]-356*sd[2]-202*sd[3];
    assign ze[5] = 202*sd[0]-356*sd[1]+ 70*sd[2]+302*sd[3];
    assign ze[7] =  70*sd[0]-202*sd[1]+302*sd[2]-356*sd[3];
