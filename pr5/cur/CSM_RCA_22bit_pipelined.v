module CSM_RCA_22bit_pipelined (
	output [44-1:0] mul,
	input [22-1:0] a, b,
	input clk, rstn
);
	...
	wire [44-1:0] mul_d;
	wire [21:0]   mul_low_q;
	wire [21:0]  mul_low_out;
	wire [21:0] mul_high_out;
	DFF_22bit DFF_a_in (.q(a_q), .d(a), .clk(clk), .rstn(rstn));
	DFF_22bit DFF_b_in (.q(b_q), .d(b), .clk(clk), .rstn(rstn));

<<<<<<< HEAD
	DFF_22bit DFF_mul_low  (.q(mul_low_out),  .d(mul_low_q),   .clk(clk), .rstn(rstn));
	DFF_22bit DFF_mul_high (.q(mul_high_out), .d(mul_d[43:22]),.clk(clk), .rstn(rstn));
=======
	multiplier_rca MUL_22_22_RCA (.mul(mul_d), .a(a_q), .b(b_q), .clk(clk), .rstn(rstn));
>>>>>>> 2dda34e30f20514a49f3153639b6780c58f9af11

	assign mul = {mul_high_out, mul_low_out};
	
	multiplier_rca MUL_22_22_RCA (.mul(mul_d), .mul_low(mul_low_q), .a(a_q), .b(b_q), .clk(clk), .rstn(rstn));
...
<<<<<<< HEAD
	...
	Carry_Save_Adder_22bit CSA19 (.sum(pp_sum[19]), .c_out(c_out[19]), .mul_out(mul[20]), .a({1'b0, pp_sum[18]}), .b(x[20]), .c_in(c_out[18]));
	//critical path
	wire [20:0] pp_sum20_q;
	wire [21:0] c_out20_q;

	Carry_Save_Adder_22bit CSA20 (.sum(pp_sum[20]), .c_out(c_out[20]), .mul_out(mul[21]), .a({1'b0, pp_sum[19]}), .b(x[21]), .c_in(c_out[19]));

	DFF_21bit DFF_pp_sum20 (.q(pp_sum20_q), .d(pp_sum[20]), .clk(clk), .rstn(rstn));
	DFF_22bit DFF_c_out20  (.q(c_out20_q),  .d(c_out[20]),  .clk(clk), .rstn(rstn));
	DFF_22bit DFF_mul_low  (.q(mul_low),    .d(mul[21:0]),  .clk(clk), .rstn(rstn));  

	//critical path
	// #3: Vector Merging Adder
	FA_22bit VMA00 (.sum(mul[43:22]), .a({1'b0, pp_sum20_q}), .b(c_out20_q), .c_in(1'b0));
=======
	// ── Pipeline cut ──────────────────────────────────────
